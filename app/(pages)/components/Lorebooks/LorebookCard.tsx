        ...
        <Link href={`/lorebooks/${lorebook.id}`}>
            ...
            >
                ...
                <div className="p-6 space-y-4 relative">
                    {/* Title */}
                    <h3 className="text-xl font-black text-white italic uppercase tracking-tight leading-tight group-hover:text-cyan-400 transition-colors line-clamp-2 min-h-[3.5rem]">
                        {lorebook.name}
                    </h3>
                    ...
                    <div className="grid grid-cols-3 gap-3 pt-3 border-t border-white/5">
                        <div className="flex flex-col items-center">
                            <span className="text-[9px] font-black text-white/20 uppercase tracking-widest mb-1">Entries</span>
                            <span className="text-lg font-black text-white italic">{lorebook._count?.entries || 0}</span>
                        </div>
                        <div className="flex flex-col items-center border-x border-white/5">
                            <span className="text-[9px] font-black text-white/20 uppercase tracking-widest mb-1">Depth</span>
                            <span className="text-lg font-black text-cyan-400 italic">{lorebook.scanDepth || 4}</span>
                        </div>
                        <div className="flex flex-col items-center">
                            <span className="text-[9px] font-black text-white/20 uppercase tracking-widest mb-1">Budget</span>
                            <span className="text-lg font-black text-white italic">{lorebook.tokenBudget || 512}</span>
                        </div>
                    </div>
                    ...
                    {lorebook.tags && lorebook.tags.length > 0 && (
                        <div className="flex flex-wrap gap-2 pt-2">
                            {lorebook.tags.slice(0, 3).map((tag: any) => (
                                <span
                                    key={tag.id}
                                    className="px-2 py-1 bg-white/5 border border-white/10 rounded-lg text-[9px] font-black text-white/60 uppercase tracking-widest hover:bg-white/10 hover:text-cyan-400 transition-colors"
                                >
                                    {tag.name}
                                </span>
                            ))}
